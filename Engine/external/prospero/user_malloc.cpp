...
#define INIT_ASAN_MUTEX()                                                              \
    do                                                                                 \
    {                                                                                  \
        if (SCE_OK != scePthreadMutexInit(&s_mtx, nullptr, "AllocatorMutex")) abort(); \
    } while (0)

#define DESTROY_ASAN_MUTEX()                                   \
    do                                                         \
    {                                                          \
        if (SCE_OK != scePthreadMutexDestroy(&s_mtx)) abort(); \
    } while (0)

#define LOCK_ASAN_MUTEX()                                   \
    do                                                      \
    {                                                       \
        if (SCE_OK != scePthreadMutexLock(&s_mtx)) abort(); \
    } while (0)

#define UNLOCK_ASAN_MUTEX()                                   \
    do                                                        \
    {                                                         \
        if (SCE_OK != scePthreadMutexUnlock(&s_mtx)) abort(); \
    } while (0)
...
int user_malloc_init(void)
{
    ...
    // J ダイレクトメモリを割り当てる
    res = sceKernelAllocateDirectMemory(0,
    ...
    res = sceKernelMapDirectMemory(&s_addr,
                                   HEAP_SIZE,
...
{
#ifdef ENABLE_ASAN
    if (SIZE_MAX - ALLOC_OFFSET < size) return nullptr;
...
void *user_aligned_alloc(size_t alignment, size_t size)
{
    ...
    if (SIZE_MAX - offset < size) return nullptr;
...
int user_posix_memalign(void **ptr, size_t boundary, size_t size)
{
    ...
    if (SIZE_MAX - offset < size) return ENOMEM;
...
int user_malloc_stats(SceLibcMallocManagedSize *mmsize) { return sceLibcMspaceMallocStats(s_mspace, mmsize); }

// E Replace malloc_stats_fast function.
// J malloc_stata_fast 関数と置き換わる
int user_malloc_stats_fast(SceLibcMallocManagedSize *mmsize) { return sceLibcMspaceMallocStatsFast(s_mspace, mmsize); }
