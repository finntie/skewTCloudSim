...
//
//			// 4.
//			const int GH = m_GHeight[i] + 1 >= GRIDSIZESKYY ? GRIDSIZESKYY - 1 : m_GHeight[i] + 1;
//			const int envGIdx = i + GH * GRIDSIZEGROUND;
...
//{
//	//Backwards calculation, moving from left to right, looking at previous.
//	if (i % GRIDSIZEGROUND == 0) return;
//	const int pX = i - 1;
//
...
//
//		const float slope = float(m_GHeight[pX] - m_GHeight[i]) / 1.0f; // Divide by 1 is useless but its to say that height changed with n by 1 meters.
//		const bool fromPrev = slope > 0.0f;
//		const float slopeFromRain = fromPrev ? m_groundGrid.Qgr[pX] : m_groundGrid.Qgr[i];
//		const float slopeFromWater = fromPrev ? m_groundGrid.Qrs[pX] : m_groundGrid.Qrs[i];
//
...
//		}
//		const float changeWater = g * k * slopeFromWater * slope;
//
//		//Limit
//		slopeFlowRain = dt * std::min(changeRain * m_speed, fromPrev ? m_groundGrid.Qgr[pX] : m_groundGrid.Qgr[i]);
//		slopeFlowWater = dt * std::min(changeWater * m_speed, fromPrev ? m_groundGrid.Qrs[pX] : m_groundGrid.Qrs[i]);
//	}
//
...
//
//	const float lapR = (m_groundGrid.Qgr[pX] - m_groundGrid.Qgr[i]) / (VOXELSIZE * VOXELSIZE) * DG;
//	const float lapSR = (m_groundGrid.Qrs[pX] - m_groundGrid.Qrs[i]) / (VOXELSIZE * VOXELSIZE) * DS;
//	const bool fromPrevR = lapR > 0.0f;
//	const bool fromPrevSR = lapSR > 0.0f;
//	//Limit
//	const float flowGroundRain = dt * std::min(lapR * m_speed, fromPrevR ? m_groundGrid.Qgr[pX] : m_groundGrid.Qgr[i]);
//	const float flowGroundWater = dt * std::min(lapSR * m_speed, fromPrevSR ? m_groundGrid.Qrs[pX] : m_groundGrid.Qrs[i]);
//
//	m_groundGrid.Qgr[pX] -= flowGroundRain + slopeFlowRain;
//	m_groundGrid.Qrs[pX] -= flowGroundWater + slopeFlowWater;
//	m_groundGrid.Qgr[i] += flowGroundRain + slopeFlowRain;
//	m_groundGrid.Qrs[i] += flowGroundWater + slopeFlowWater;
//}
//
...
//	if (m_envGrid.Qr[iH] > 0.0f || m_envGrid.Qs[iH] > 0.0f || m_envGrid.Qi[iH] > 0.0f) fallVel = calculateFallingVelocity(iH, density, 4);
//
...
//				const float l = m_NeighData[j].getOutsideLeft() ? getIsentropicTemp(yPos) : m_NeighData[j].getOutsideLeft() == GROUND && isGroundLevel(idxGL, int(yPos)) ? meteoformulas::potentialTemp(float(m_groundGrid.T[idxGL]) - 273.25f, m_pressures[int(yPos)], m_groundGrid.P[idxGL]) + 273.15f : dif[j - 1];
//				const float r = m_NeighData[j].getOutsideRight() ? getIsentropicTemp(yPos) : m_NeighData[j].getEnvTypeRight() == GROUND && isGroundLevel(idxGR, int(yPos)) ? meteoformulas::potentialTemp(float(m_groundGrid.T[idxGR]) - 273.25f, m_pressures[int(yPos)], m_groundGrid.P[idxGR]) + 273.15f : dif[j + 1];
//				const float d = m_NeighData[j].getEnvTypeDown() == GROUND ? meteoformulas::potentialTemp(float(m_groundGrid.T[idxGD]) - 273.25f, m_pressures[int(yPos)], m_groundGrid.P[idxGD]) + 273.15f : dif[j - GRIDSIZESKYX];
//				const float u = m_NeighData[j].getOutsideUp() ? getIsentropicTemp(yPos) : dif[j + GRIDSIZESKYX];
//
...
//					{
//						if (m_NeighData[j].getEnvTypeDown() == GROUND) { dif[j] = array[j]; continue; }
//						const float left = m_NeighData[j].getOutsideLeft() ? getIsentropicVapor(yPos) : m_NeighData[j].getOutsideLeft() == GROUND ? dif[j] : dif[j - 1];
//						const float right = m_NeighData[j].getOutsideRight() ? getIsentropicVapor(yPos) : m_NeighData[j].getEnvTypeRight() == GROUND ? dif[j] : dif[j + 1];
//						const float down = m_NeighData[j].getEnvTypeDown() == GROUND ? dif[j] : dif[j - GRIDSIZESKYX];
//						const float up = m_NeighData[j].getOutsideUp() ? getIsentropicVapor(yPos) : dif[j + GRIDSIZESKYX]; //Neumann due to being diffusion
//					
//						dif[j] = (array[j] + k * (left + right + up + down)) / (1 + 4 * k);
//					}
//					else // Else use nuemann
//					{
//						const float left = m_NeighData[j].getOutsideLeft() ? dif[j] : m_NeighData[j].getOutsideLeft() == GROUND ? dif[j] : dif[j - 1];
//						const float right = m_NeighData[j].getOutsideRight() ? dif[j] : m_NeighData[j].getEnvTypeRight() == GROUND ? dif[j] : dif[j + 1];
//						const float down = m_NeighData[j].getEnvTypeDown() != SKY ? dif[j] : dif[j - GRIDSIZESKYX];
//						const float up = m_NeighData[j].getOutsideUp() ? 0.0f : dif[j + GRIDSIZESKYX];
//	
//						dif[j] = (array[j] + k * (left + right + up + down)) / (1 + 4 * k);
//					}
//	
...
//				fallVelocitiesPrecipUP = calculateFallingVelocity(iUP, density, fallVelType);
//			}
//			switch (fallVelType)
//			{
//			case 1:
...
//			}
//			array[i] -= dt * std::min((fallVel / VOXELSIZE) * array[i], array[i]); //Remove % of precip
//
//			if (array[i] < 0.0f)
//			{
//				fallVel = 0.0f;
...
//
//	if (m_NeighData[i].getOutsideRight())
//	{
//		return;
//	}
...
//		if (m_NeighData[i].getOutsideLeft() || m_NeighData[i].getOutsideRight() || m_NeighData[i].getOutsideUp() || m_NeighData[i].getOutsideDown())
//		{
//			continue;
//		}
//	
...
//	
//					const glm::vec2 left  = m_NeighData[i].getOutsideLeft()  ? dif[i] : m_NeighData[i].getEnvTypeLeft() == GROUND ? glm::vec2(0.0f) : dif[i - 1];	// Neumann (Or no slip)
//					const glm::vec2 right = m_NeighData[i].getOutsideRight()  ? dif[i] : m_NeighData[i].getEnvTypeRight() == GROUND ? glm::vec2(0.0f) : dif[i + 1];	// Neumann (Or no slip)
...
//	const float potTempMultiplier = glm::pow((m_pressures[y] / m_groundGrid.P[0]), Constants::Rsd / Constants::Cpd);
//	const int distOYY = (oY - y) * (oY - y);
//
//	float amount = 0.0f;
//	float average = 0.0f;
//
...
//	{
//		if (x >= GRIDSIZESKYX || isGround(x, y)) break;
//
...
//		if (temp) average += float(m_envGrid.potTemp[x + y * GRIDSIZESKYX]) * potTempMultiplier * cAmount;
//		else average += m_envGrid.Qv[x + y * GRIDSIZESKYX] * cAmount;
//	}
//	for (int x = oX - 1; x > minLeft; x--)
//	{
//		if (x < 0 || isGround(x, y)) break;
//
//		const int distOXX = oX - x;
//		const float distance = float(distOXX * distOXX + distOYY);
//		float cAmount = -(distance * MDistanceSqr - 1);
...
//	const float left  = m_NeighData[i].getOutsideLeft() ? center : m_NeighData[i].getEnvTypeLeft() == GROUND ? center : curl(i - 1);	// Neumann
//	const float right = m_NeighData[i].getOutsideRight() ? center : m_NeighData[i].getEnvTypeRight() == GROUND ? center : curl(i + 1);	// Neumann
...
//
//	const glm::vec2 v00 = outside(x, y)			|| isGround(int(x), int(y))			|| m_NeighData[index00].getOutsideRight() ? glm::vec2(m_defaultVel[int(y)], 0.0f) : m_envGrid.velField[index00];
//	const glm::vec2 v10 = outside(x + 1, y)		|| isGround(int(x) + 1, int(y))		|| m_NeighData[index10].getOutsideRight() ? glm::vec2(m_defaultVel[int(y)], 0.0f) : m_envGrid.velField[index10];
//	const glm::vec2 v01 = outside(x, y + 1)		|| isGround(int(x), int(y) + 1)		|| m_NeighData[index01].getOutsideRight() ? glm::vec2(m_defaultVel[int(y)], 0.0f) : m_envGrid.velField[index01];
//	const glm::vec2 v11 = outside(x + 1, y + 1) || isGround(int(x) + 1, int(y) + 1) || m_NeighData[index11].getOutsideRight() ? glm::vec2(m_defaultVel[int(y)], 0.0f) : m_envGrid.velField[index11];
//
//
//	//Interpolate using bilinear interpolation, could be compressed
//	if (U)
...
//		if (isGround(i)) continue;
//		float Uneg = 1.0f;
//		const float Ucurr = ((m_NeighData[i].getOutsideRight()) && i != 0) ? m_defaultVel[i / GRIDSIZESKYX] : (m_NeighData[i].getEnvTypeRight() == GROUND ? 0.0f : m_envGrid.velField[i].x);
//		const float Umin1 = (m_NeighData[i].getOutsideLeft() || i == 0) ? m_defaultVel[i / GRIDSIZESKYX] : (m_NeighData[i].getEnvTypeLeft() == GROUND ? 0.0f : m_envGrid.velField[i - 1].x);
...
//			const int Aplusx = x == GRIDSIZESKYX - 1 ? 0 : A[idx].x; //looking at x 
...
//			const float Splusx = x == GRIDSIZESKYX - 1 ? s[idx] : s[idx + 1];
...
//}
//
//glm::vec3 environment::calculateFallingVelocity(const int i, const float densAir, const int type)
//{
//	bool all{ (type == 4) };
//	float UR = 0.0f;
//	float US = 0.0f;
...
//		UR = a * (gammaR / (6 * std::pow(slopeR, b))) * sqrt(1.225f / densAir);
...
//		US = c * (gammaS / (6 * std::pow(slopeS, d))) * sqrt(1.225f / densAir);
...
//		UI = (gammaI / (6 * std::pow(slopeI, 0.5f))) * pow(4 * g * 100 * densI / (3 * CD * densAir * 0.001f), 0.5f); //Converting g to cm/s2 and densAir to g/cm3
...
//{
//	if (m_NeighData[i].getOutsideRight()) return;
//	const int idxX = i % GRIDSIZESKYX;
//	const int idxY = i / GRIDSIZESKYX;
//
//	glm::vec3 fallVel{ 0.0f };
//	if (m_envGrid.Qr[i] > 0.0f || m_envGrid.Qs[i] > 0.0f || m_envGrid.Qi[i] > 0.0f) fallVel = calculateFallingVelocity(i, density, 4);
//
...
//}
//
//bool environment::isGroundLevel(int i)
//{
//	int x = i % GRIDSIZESKYX;
//	int y = i / GRIDSIZESKYX;
//	return y == m_GHeight[x] - 1;
//}
//
//bool environment::isGroundLevel(int x, int y)
//{
//	return y == m_GHeight[x] - 1;
//}
//
...
//	for (int x = 0; x < GRIDSIZEGROUND; x++)
//	{
//		const int GH = m_GHeight[x] + 1 >= GRIDSIZESKYY ? GRIDSIZESKYY - 1 : m_GHeight[x] + 1;
//		const float T = meteoformulas::potentialTemp(getIsentropicTemp(float(GH - 1)) - 273.15f, m_groundGrid.P[x], m_pressures[GH]);
