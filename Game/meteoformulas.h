...
public:
	...
	/// <summary>Get Saturated Vapor Pressure, also sometimes called Psat
	/// https://www.weather.gov/media/epz/wxcalc/vaporPressure.pdf</summary>
	/// <param name="T">Temperature in C</param>
	/// <returns>saturation vapor pressure in hPa</returns>
	static float es(const float T);


	/// <summary>Calculates the saturated mixing ratio 
	/// https://vortex.plymouth.edu/~stmiller/stmiller_content/Publications/AtmosRH_Equations_Rev.pdf</summary>
	/// <param name="T">Temperature in C</param>
	/// <param name="P">Pressure in hPa</param>
	/// <returns>ws in (kg(vapor)/kg(air))</returns>
	static float ws(const float T, const float P);

	/// <summary>Calculates the saturated mixing ratio (rs) (same as above, but using a different function)
	/// https://journals.ametsoc.org/view/journals/mwre/108/7/1520-0493_1980_108_1046_tcoept_2_0_co_2.xml</summary>
	/// <param name="T">Temperature in C</param>
	/// <param name="P">Pressure in hPa</param>
	/// <returns>ws in (kg(vapor)/kg(air))</returns>
	static float rs(const float T, const float P);

	/// <summary>Calculates the specific humidity </summary>
	/// <param name="T">Temperature in °C</param>
	...
	static float getEL(const float T0, const float P0, const float Z0, const float D0, const skewTInfo& OData);
